...
bool OTAUpdate::process(uint8_t *data, size_t len) {
    size_t index = 0;
    bool retval = true;
            ...
            case State::HEADER:
                ...
                if (_loc == sizeof(efuheader_t)) {
                    if (_header.signature == EFU_ID) {
                        // TODO: uncomment _header.version = ntohs(_header.version);
                        memset(&_record, 0, sizeof(efurecord_t));
                        _loc = 0;
                        _state = State::RECORD;
                    } else {
                        _state = State::FAIL;
                        _error = UPDATE_ERROR_SIG;
                    }
                }
                break;
            case State::RECORD:
                ...
                if (_loc == sizeof(efurecord_t)) {
                    // TODO: uncomment _record.type = RecordType(ntohs((uint16_t)_record.type));
                    // TODO: uncomment _record.size = ntohl(_record.size);
                    _loc = 0;
...
bool OTAUpdate::hasError() {
    return _error != UPDATE_ERROR_OK;
}

uint8_t OTAUpdate::getError() {
    return _error;
}
